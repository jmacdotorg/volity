...
__PACKAGE__->uri("http://volity.org/games/guess100");
...
sub handle_chat_message {
  ...
  my $chatter_jid = $self->referee->look_up_jid_with_nickname($$message{from});
  ...
  if (my ($guess) = $body =~ /^\s*(\d+)\s*$/) {
    ...
    $self->referee->groupchat($player->nick . " made a guess: $guess.");
    if ($guess == $self->the_number) {
      # The game is over.
	$self->referee->groupchat("And that is correct! " . $player->nick . " wins!");
      $self->end_game({status=>"completed", winners=>[$player]});
    } elsif ($guess < $self->the_number) {
      # The guess is too low.
	$self->referee->groupchat("But that number is too low.");
      # Move to the next player.
      $self->end_turn;
    } else {
      # The guess is too hah.
      $self->referee->groupchat("But that number is too high.");
      # Move to the next player.
      $self->end_turn;
    }
  } else {
...
sub end_turn {
  ...
  $self->referee->groupchat("It is now " . $self->current_player->nick . "'s turn.");
}
