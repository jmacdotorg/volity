 ...
 * @toc-sort 1/2
 */
class org.jivesoftware.xiff.data.whiteboard.Path implements ISerializable
{
	...
	public function Path( parent:XMLNode )
	{
		super( parent );
	...
	{
        var node = XMLStanza.XMLFactory.createElement(Path.ELEMENT);
	...
	private static function indexOfNextCommand(str):Number
	{
	...
	private function loadNextCommand(str):Void
	{
		// Example
		...
		var idx = indexOfNextCommand(str);
		if (idx >= 0) {
			var cmd = str.charAt(idx);
			var commands = str.split(cmd);
	...
	private function loadCommand(cmd, str)
	{
